...
public class RegistroHuesped extends JFrame {
	...
	public int random = (int) (Math.random() * (9999 - 1000 + 1)) + 1000;
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					RegistroHuesped frame = new RegistroHuesped( reservasView);
	...
	public RegistroHuesped(ReservasView reservasView) {
		...
		txtNreserva.setFocusable(false);
		...
		btnguardar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				try (Connection connection = connectionFactory.recuperaConexion()) {
					...
					Date fechaNac = new java.sql.Date(txtFechaN.getDate().getTime());
